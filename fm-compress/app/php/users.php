    ...
    if (isset($_POST['createUserButton'])) {
        ...
        if (!empty($_FILES['profilePicturefield'])) {
            ...
            if (!move_uploaded_file($tmp_name, $uploads_dir . $randomName)) {
                echo "Something was wrong with file1"; //para que funcione la foto no tiene que estar en el servidor
            }
        } else {
            $randomName = "default.png";
            if (!move_uploaded_file($tmp_name, $uploads_dir . $randomName)) {
                echo "Something was wrong with file2"; //para que funcione la foto no tiene que estar en el servidor
            }
        }
        ...
        if ($password == $repassword) {
            if (checkPassword($password)) {
                $checkUser = array($name, $lastname, $username, $password, $email, $officeNumber, $phoneNumber, $profilePicture, $faculty, $role);
                if (checkEmptyFields($checkUser)) {
                    //encrytp password with md5
                    $encryptedPass = md5($password);
                    $newUser = new User($name, $lastname, $username, $encryptedPass, $email, $officeNumber, $phoneNumber, $profilePicture, $faculty, $role);
